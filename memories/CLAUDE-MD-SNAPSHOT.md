...
- **已学习 Skill — image-to-video**：通过 each::sense API 将静态图片转为动态视频（需 EACHLABS_API_KEY），支持 Ken Burns 效果、Cinemagraph、产品展示动画、3D Parallax、Logo 动画等
- **已学习 Skill — video-stabilization**：通过 each::sense API 稳定抖动视频（需 EACHLABS_API_KEY），可用于教学视频的后期防抖处理
- **已学习 Skill — video-noise-reduction**：通过 each::sense API 降低视频噪点（需 EACHLABS_API_KEY），可用于低光环境拍摄的教学视频降噪处理
- **已学习 Skill — video-format-conversion**：通过 each::sense API 转换视频格式（需 EACHLABS_API_KEY），支持多种视频格式互转，可用于教学视频的格式适配与分发
- **已学习 Skill — video-speed-adjustment**：通过 each::sense API 调整视频速度（需 EACHLABS_API_KEY），支持慢动作、快进、变速等效果，可用于教学视频的节奏控制
- **已学习 Skill — video-localization**：通过 each::sense API 进行视频本地化（需 EACHLABS_API_KEY），支持字幕翻译、配音替换等，可用于教学视频的多语言版本制作
- **已学习 Skill — image-upscaling**：通过 each::sense API 提升图片分辨率（需 EACHLABS_API_KEY），可用于教学视频缩略图和素材的高清化处理
- **已学习 Skill — video-trimming**：通过 each::sense API 裁剪视频片段（需 EACHLABS_API_KEY），支持精确剪辑、片段提取，可用于教学视频的精剪
- **已学习 Skill — video-watermark**：通过 each::sense API 添加视频水印（需 EACHLABS_API_KEY），支持品牌 Logo 水印、文字水印，可用于 Peblla 品牌保护
- **已学习 Skill — youtube-video-generation**：通过 each::sense API 生成 YouTube 视频（需 EACHLABS_API_KEY），支持从文本/脚本自动生成视频内容
- **已学习 Skill — remotion**：Remotion 程序化视频制作（React），可用于批量生成统一片头片尾、自动字幕烧录、模板化转场、音频可视化等教学视频的重复性制作工作
...
- 当前平台：YouTube
- **已学习 Skill — youtube-plan-new-video**：YouTube 视频策划全流程，编排标题/缩略图/Hook 生成，基于研究数据产出制作就绪的视频计划（含 AB 测试缩略图）
- **已学习 Skill — youtube-seo**：YouTube 视频与频道搜索优化指南，涵盖标题（<60字符+关键词前置）、描述（关键词自然融入+160字符 meta）、标签、缩略图策略、Title+Description+Thumbnail 协同优化，以及 YouTube 在 Google 搜索和 AI Overviews/GEO 中的引用优化策略
...
- ⚠️ 关键规则：数据必须最新（每周重新实测）；数据自己收集（不依赖他人）
...
- 详细 SEO 知识：见 blogger_seo_knowledge.md
...
- **Framer 登录账号**：zhangpengwenjia@alayitech.com
- **操作权限（永久，严格遵守）**：
...
- **产品信息查证规则（永久·Kristin 要求）**：
  ...
  - 飞书知识库入口：通过 Kristin 的飞书 MCP（`feishu-mcp-url`）搜索 wiki
...
- **询盘路径分析**（每有新询盘时触发）：
  ...
  - Step 3：交叉验证 — 表单追踪字段 × GA4 站内路径，拼出完整链路
  - 输出格式：🔗流量来源 → 🌐着陆页 → 📍城市 → 🗺️站内路径（哪些页面→每页几次→在哪提交）→ 📈路径解读与意向判断
...
- **私有化看板方案**（Cloudflare R2 + Worker + OTP）：
  - **已配置完成**：域名 `mkt-dashboard.peblla.com`，Bucket `peblla-marketing-bucket`
  - 凭证保存在 `.r2-config`，上传脚本 `upload.sh`（boto3），已验证可用
  - 架构：本地生成 HTML → `bash upload.sh <文件> <R2路径>` → Worker 拦截请求 → 按 URL 路径前缀判断权限
  - **强制规则：所有看板/报告一律上传到 `private/` 目录**（有鉴权保护）
  - R2 三目录规则：`private/`（OTP 邮箱验证，仅公司域名）、`partner/`（OTP，公司+合作方域名）、`public/`（任何人可访问），不确定放哪→一律 `private/`
  - OTP 流程：访问 → 无 session cookie → 输入邮箱 → 验证域名白名单 → 发 6 位 OTP（Mailgun/Resend/SES）→ 验证通过 → 设 session cookie（7天）
  - 安全：OTP 5 分钟有效、HTTPS、R2 不公开仅 Worker 代理、排除内部工具域名污染 GA4
  - 上传：`bash upload.sh <本地文件> <R2路径>`，自动检测 Content-Type
  - 自动更新：数据变更 → 脚本生成 HTML → upload.sh 上传 R2 → 团队同一链接看最新数据
  - 完整代码示例见 `virtual-team-full.md`
...
- **Logo 本地素材库（永久·Amanda Zhu 提供）**：`./shared/peblla-logo/01 Peblla Logo/`，含 Symbol Mark（icon）PNG×6 + WordMark PNG×5 + AI 矢量源文件×4，制作 marketing 材料时优先从此目录调用，不需要去官网抓取
...
**审核流程（永久·强制·张彭文佳要求）**：所有角色生成的图片/视频，在发给群里之前**必须自动走 Safer 审核工作流**，不需要人工提醒。流程：生图 → Safer 自查（硬件外观是否匹配 SUNMI 原型、logo 是否去除、品牌元素是否正确、画面质量是否达标）→ 审核通过才发群。审核不通过则自行修正后重新审核，直到通过为止。**任何图片/视频未经 Safer 审核一律不得发出。**
...
- **产品规格**：
  - **Postcard**: 4in × 6in（用于 first order promotion，放餐厅前台供顾客取用）
  - **Poster**: 24in × 36in（张贴店内墙面/窗户）
...
- **设计制作规范（Amanda Zhu 培训）**：
  ...
  - **Example 参考**：`shared/marketing-kit-examples/HH tea - Baltimore, MD/`（含 4×6 postcard + 24×36 poster + Maps 缩略图）
  - **永远竖版（vertical·永久·Amanda Zhu 要求）**：无论 4×6 postcard 还是 24×36 poster，一律竖版（portrait），不做横版
  - **信息量充足原则（永久·Amanda Zhu 训练）**：画面不能只有一条促销信息，要用有营销价值的文案填满画面。参考 HH Tea——除了 "First Order X% OFF" 还要有 "Free Delivery"、"Download our APP"、"Order @ website-URL"、"GET APP" 指向 QR 等引导性文案
  - **QR Code 要够大**：4×6 postcard 上 QR 至少 1.2 inch（360px @300dpi），确保实物手机能扫
  - **Powered by Peblla**：缩小放右下角，不抢眼（Amanda 确认风格）
  - **不做横版小卡片（永久·Amanda Zhu 要求·2026-03-27）**：Marketing Kit 只需 4×6 Postcard + 24×36 Poster，不再制作横版 Business Card 正面促销卡（card_front）
...
**公司**：Peblla / Rosper
**纸张规格**：
- 标准纸（Standard）：3.5 × 2 inches → EGI Printing
- 高级纸（Premium）：3.3 × 2.16 inches → MOO (moo.com)
...
- **字体**：Roboto-Medium（`Roboto-Medium.ttf`，已下载至 session 目录）
